...
HSD_GObj* fn_802F77F8(HSD_GObj* gobj, u8 slot, u16 arg2)
{
    ...
    if (gobj != NULL) {
        /* scene_models[0] is a 32-bit disc slot holding the
         * DynamicModelDesc, and the desc's own `joint` is another 32-bit
         * slot. Both need relocating -- reading them as host pointers loads
         * 8 bytes of adjacent disc data and hands HSD_JObjLoadJoint a wild
         * address. */
        DynamicModelDesc* desc =
            DP(DynamicModelDesc, ((const DiscU32*) models->scene_models)->v);
