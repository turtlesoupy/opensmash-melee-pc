...
void vi1101_Scene_OnEnter(void* arg)
{
    ...
    cobj = lb_80013B14(vi_SceneCamDesc(scene));
    HSD_GObjObject_80390A70(camera_gobj, HSD_GObj_CameraKind, cobj);
    GObj_SetupGXLinkMax(camera_gobj, fn_8031F56C, 5);
    HSD_CObjAddAnim(cobj, vi_SceneCamAnim(scene, 0));
    HSD_CObjReqAnim(cobj, un_804DE0DC);
    HSD_CObjAnim(cobj);
    HSD_GObj_SetupProc(camera_gobj, fn_8031F600, 0);

    for (i = 0; vi_SceneModel(un_804D6FC0, i) != NULL; i++) {
        DynamicModelDesc* model = vi_SceneModel(un_804D6FC0, i);
        HSD_Joint* joint;
        u8 jobj_kind;
        HSD_JObj* jobj_copy;

        model_gobj = GObj_Create(0xE, 0xF, 0);
        jobj = HSD_JObjLoadJoint(joint = DP(HSD_Joint, model->joint));
