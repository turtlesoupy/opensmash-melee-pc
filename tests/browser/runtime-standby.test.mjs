...
test('standby initializes the engine once, then installs assets before resuming it',async()=>{
 const events=[],writes=[],parent={postMessage:data=>events.push(data)};
 let receive,started=0,resumed=false,configured=false;
 ...
 Object.assign(module,{ENV:{},FS:{mkdirTree(){},mount(){},filesystems:{IDBFS:{}},syncfs(_read,done){done();},writeFile(path){writes.push(path);}},
  callMain(){started++;assert.equal(module.ENV.MELEE_SEED,'7');assert.ok(module.readDisc);assert.ok(writes.includes('/initial_pipeline_cache.db'));
   module.onEnginePrepared().then(()=>{assert.ok(configured);assert.ok(writes.includes('/mod/PlMrNr.dat'));resumed=true;});},
 ...
 assert.equal(started,1);assert.equal(resumed,false);assert.equal(configured,false);
 ...
 await Promise.resolve();assert.equal(resumed,true);assert.equal(started,1);
